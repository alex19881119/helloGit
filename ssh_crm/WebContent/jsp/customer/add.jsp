...
//参数二：将下拉选放入的标签id
//参数三：生成下拉选时，select标签name值
//参数四：需要回显时选中哪个option
	
	$(document).ready(function(){
		loadSelect("002","source","cust_source.dict_id" <c:if test="${customer.cust_source.dict_id!=null}">,${customer.cust_source.dict_id}</c:if>);
		loadSelect("001","industry","cust_industry.dict_id" <c:if test="${customer.cust_industry.dict_id!=null}">,${customer.cust_industry.dict_id}</c:if>);
		loadSelect("006","level","cust_level.dict_id" <c:if test="${customer.cust_level.dict_id!=null}">,${customer.cust_level.dict_id}</c:if>);
		});
